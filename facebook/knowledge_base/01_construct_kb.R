...
if(any(duplicated(people$wmpid))){stop("There are duplicate people.")}
...
if(any(duplicated(cands$wmpid))){stop("There are duplicate candidates.")}
...
people$first_name_extracted <- unlist(lapply(tks, function(x){x[1]}))

#---- LAST NAME
#If the name consists of two words, then the second one is the last name
people$last_name_extracted <- unlist(lapply(tks, function(x){if(length(x)==2){x[2]}else{NA}}))
#If the name consists of more than two words, then the last one is the last name
last_name_temp <- unlist(lapply(tks, function(x){if(length(x)>2){x[length(x)]}else{NA}}))
...
last_word_temp <- unlist(lapply(tks, function(x){x[length(x)]}))
...
jr_temp_last_names <- unlist(lapply(jr_temp_names_without_suffix_tks, function(x){x[length(x)]}))
...
II_temp_last_names <- unlist(lapply(II_temp_names_without_suffix_tks, function(x){x[length(x)]}))
...
tks_middle_names <- lapply(tks_middle_names, function(x){x[-1]}) #remove the first word
tks_middle_names <- lapply(tks_middle_names, function(x){x[-length(x)]}) #remove the last word
