...
# This is for ad tone, so we exclude disclaimer and page_name
...
# Out
path_finished_enties <- "../data/detected_entities_fb22_for_ad_tone.csv.gz"
...
# Split the id into id and field
el <- separate(el, id, c("id", "field"), "__")
# Omit disclaimer for ad_tone
el <- el %>% filter(!field %in% c("disclaimer", "page_name"))
